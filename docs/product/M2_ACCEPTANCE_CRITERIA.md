# M2 Brammel vertical-slice acceptance criteria
...
- one synthetic D&D 2024-style ruleset profile;
- one original synthetic class path sufficient for Brammel, levels 1–2 at minimum, with a deterministic route extensible to 20;
- one synthetic origin path and equipment package;
- guided and flexible level-1 creation using the nine-step flow;
- persisted draft, resume, autosave receipts, review, and character version/restore point;
...
- character transfer preview/import with no automatic cloud sync;
...
- broad class/species/background/spell support;
- multiclassing;
- automatic sync, accounts, campaigns, or VTT integration;
...
Given a playable Brammel, when its primary row is activated, then the active sheet opens; Edit, Level up, Duplicate, Export/Transfer, and Archive remain secondary actions.
...
Given guided completion criteria are met, Finish and open sheet creates a new character version and restore point in one safe transaction.

Given a flexible-playable draft with acknowledged issues, Open sheet with issues preserves those issues and makes them visible on the sheet.
...
Given playable level-1 Brammel, when Level up begins, then a pre-level restore point is identified before edits.

The flow shows automatic gains, only newly required choices, resource/equipment impacts, and a before/after diff.
...
Restricted packs/sources/entries are excluded by default and require a separate explicit export confirmation.
...
No path is called sync, no network is required for file transfer, and cancellation leaves local records unchanged.
...
| --- | --- |
| Pure unit | step planning, recommendation ranking, issue severity, derived explanations, transfer conflict plan, level-up diff |
| Dexie integration | draft autosave, transaction rollback, version-before-replace, missing source, duplicate/conflict, restricted export |
...
Before M2 implementation is declared complete, all contributor-guide commands must pass:
...
M2 implementation is ready to start only when:

- the open design questions below have owners or explicit deferrals;
- the Brammel synthetic content required by the slice is licensed/original and enumerated;
- service boundaries for drafts, character versions, snapshots, runtime mutations, and transfers are agreed without modifying M1 privacy guarantees;
- a 360 px low-fidelity prototype validates the nine-step shell and Play home with representative long labels;
- test fixtures contain no official non-SRD or private content.

## Open design questions

1. What is the minimum synthetic Brammel rules/content set for a meaningful level 1→2 slice, and should “Boss” be represented as identity only or as an original feature?
2. What exact fields constitute the minimum renderable play shell versus a blocked calculation?
3. Should flexible mode allow opening a sheet with no class, or require a neutral manual character type?
4. Are overrides absolute replacements, additive modifiers, or both per target, and how are their scopes represented in the existing domain model?
5. Which runtime changes belong in character history versus a lighter session-action log?
6. How should current HP/resources change when a level-up increases maxima?
7. Is QR transfer in the first implementation, or is file transfer the acceptance baseline with QR deferred?
8. Which portable character dependencies can be embedded in a standard export without crossing the restricted-content confirmation boundary?
9. At 1024 px should the default shell use persistent navigation or retain a distraction-free tablet mode?
10. Which dice behavior belongs in the Brammel slice: displayed expression only, local random result, or a fuller roll log?

## GO / NO-GO gate

**Recommendation: GO for a constrained Brammel interaction prototype and service-boundary design; NO-GO for broad M2 production implementation yet.**

The research supports the product direction and the existing architecture supports a local-first slice. Broad implementation should wait until questions 1–4 and 8 are answered, the mobile overflow is treated as a release-blocking regression, and transfer/override semantics are agreed. This avoids encoding ambiguous product rules into schemas or persistence prematurely.
