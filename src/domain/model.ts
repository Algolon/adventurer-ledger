...
export type Category="class"|"class-feature"|"subclass"|"species"|"race"|"lineage"|"background"|"feat"|"spell"|"item"|"weapon"|"armor"|"tool"|"fighting-style"|"weapon-mastery"|"maneuver"|"invocation"|"metamagic"|"infusion"|"pact-boon"|"condition"|"resource"|"rule";
...
export interface ContentPack extends Audit{id:ID;name:string;description?:string;version:string;schemaVersion:number;rulesEditions:Edition[];visibility:"public"|"private";licenseType:LicenseType;exportRestricted:boolean;includeFullText:boolean;sourceIds:ID[];entryIds:ID[];checksum?:string}
export interface ContentEntry extends Audit{id:ID;slug:string;name:string;aliases:string[];category:Category;subcategory?:string;rulesEdition:Edition;sourceId:ID;sourceBook?:string;sourcePage?:string;sourceSection?:string;licenseType:LicenseType;visibility:Visibility;fullText?:string;summary?:string;playerNotes?:string;dmNotes?:string;prerequisites:PrerequisiteDefinition[];choices:ChoiceDefinition[];effects:Effect[];tags:string[];version:string;revision:number;errataVersion?:string;replacementOf?:ID;replacedBy?:ID;legacy:boolean;optional:boolean;private:boolean;exportRestricted:boolean}
