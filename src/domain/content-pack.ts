...
const mechanicsByCategory = {
   ...
   *
   * Metadata, and only metadata: nothing casts a ritual yet. It is typed and
   * defaulted rather than optional so the answer is never "the record does not
   * say" — every parsed spell carries `true` or `false`, a record written before
   * the field existed keeps validating, and the fact stops living in prose,
   * where no reader can act on it.
   *
   * Deliberately narrow. A component model rich enough to express which material
   * requirements a class table singles out is separate, larger work; folding it
   * in here would bake a guess into stored content.
   */
  ritual: z.boolean().default(false) }).strict(),
