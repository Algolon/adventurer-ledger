...
export class SourceRepository {
  ...
  async update(id: string, changes: Partial<Omit<Source, "id" | "createdAt">>) {
    ...
    if (!current) throw new Error(`Source ${id} was not found`);
    const next = { ...current, ...changes, id, updatedAt: timestamp() };
    await this.database.sources.put(next);
    return next;
  }
  async delete(id: string) {
    if (await this.database.contentEntries.where("sourceId").equals(id).count())
      throw new Error(`Source ${id} is still referenced`);
...
export class ContentPackRepository {
  ...
  async create(pack: ContentPack) {
    if (
      ...
      throw new Error(`Content pack ${pack.id} already exists or is archived`);
  ...
  async update(
    ...
    return this.database.transaction(
      ...
      async () => {
        const current = await this.get(id);
        if (!current) throw new Error(`Content pack ${id} was not found`);
...
export class ContentEntryRepository {
  ...
  async update(
    ...
    return this.database.transaction(
      ...
      async () => {
        const current = await this.get(id);
        if (!current) throw new Error(`Content entry ${id} was not found`);
        const archivedAt = timestamp();
