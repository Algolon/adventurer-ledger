...
async function finishRemainingSteps(page: Page, options: { skills: readonly string[]; subclass: RegExp }) {
  ...
  for (let attempt = 0; attempt < 16; attempt += 1) {
    ...
    /*
     * Wait for a way forward, not for the step counter.
     *
     * The counter is how progress is *detected*, and it is briefly detached
     * while the builder swaps one step for the next — so requiring it before
     * acting made a transition into a failure. What has to be on screen is a
     * control: either Continue, or the Finish that ends the sequence.
     */
